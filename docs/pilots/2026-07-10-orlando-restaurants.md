...
large hospitality-group properties). 6 candidates surfaced across Mills 50,
Ivanhoe Village, and Downtown; all confirmed as real, independently or
...
Full research summaries, pain-point hypotheses, and all 18 drafted emails
...
- All 6 exist as real GHL contacts (ids above), in the `unKAGEd Media Agency`
...
- **Custom field data did not attach.** The upsert call for White Wolf Cafe
  ...
  GHL silently drops unknown keys rather than creating them. The remaining 5
...
- **No opportunities were created.** The GHL MCP connector available this
  ...
  Either create these 6 manually in the Digital Marketing Pipeline's "New
...
- **Nothing will send.** `outreach:review` is not the trigger tag for the
  ...
  `outreach:queued`) — these 6 sit inert until a human retags them.
...
3. Two of six contacts (Santiago's Bodega, Pig Floyd's) only have a
